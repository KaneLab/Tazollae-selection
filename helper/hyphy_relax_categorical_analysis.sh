...
Rscript --vanilla ~/scripts/helper/binom_test_p_val.R x=$x n=$n p=$percent outfile=p_val_col.tmp
...
cat ~/data/main_pipeline/annotations/eggnog-mapper_out/hog_annotations/hog_annotations.tsv | tail -n +2 | awk -F "\t" '$5!="-" {print $1"\t"$5}' > hogs_w_pathway.tmp
