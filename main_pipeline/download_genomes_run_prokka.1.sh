...
mkdir ~/data/ncbi_genomes
cd ~/data/ncbi_genomes
...
mkdir ~/data/main_pipeline
cd ~/sandbox
outdir=~/data/main_pipeline/prokka_out
mkdir $outdir

## Before running prokka, some of the names that I am using for RefSeq genomes (as in ~/Nostocales_incl0708.prefix.list)
## are too long. Prokka needs them to be max 23 characters.
## I therefore made a 2-col list for ~/Nostocales_incl0708.prefix.list and ~/outgroup.prefix.list
## with the full camelCase name that I am using and then a shortened version of that name.
## in ~/Nostocales_incl0708.shortened.tsv ~/outgroup.shortened.tsv
...
echo -e "\
~/data/refseq_Nostocales_genomes/data\t\
~/Nostocales_incl0708.prefix.list
~/data/fastas/Dijkhuizen_genomes\t\
~/Dijkhuizen.prefix.list
~/data/outgroup_refseq\t\
~/outgroup.prefix.list\
" | \
