bwa index ~/data/fastas/reference.fna

cat MAGList | while read sample; do

	bwa mem -t 24 ~/data/fastas/reference.fna ~/data/fastas/Dijkhuizen_genomes/$sample | samtools view -bhu - | samtools sort -@ 24 - -o ${sample}.sorted.bam
