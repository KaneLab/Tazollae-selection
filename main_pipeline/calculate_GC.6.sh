...
file=~/data/main_pipeline/prokka_out/${long_prefix}/${short_prefix}.fna
...
cat ~/data/main_pipeline/orthofinder_out/input_fastas/${short_prefix}.fasta | grep -c "^>" >> $outfile
## Get the total length of predicted intact genes
cat ~/data/main_pipeline/pseudofinder_out/${long_prefix}/${long_prefix}_intact.ffn | grep -v "^>" | tr -d "\n" | wc -c >> $outfile
## Get the number of predicted pseudogenes
cat ~/data/main_pipeline/pseudofinder_out/${long_prefix}/${long_prefix}_pseudos.gff | grep -v "^#" | wc -l >> $outfile
...
pruned_tree=~/data/main_pipeline/hyphy_out/concatenated_msas/single_copy_HOGs.pruned_tree.txt
R_outfile=n_intact_ORFs_tree.txt
table_path=n_intact_ORFs.tmp
Rscript --vanilla ~/scripts/helper/ape_ace_continuous.R table_path=$table_path tree_path=$pruned_tree outfile=$R_outfile
...
echo "~/subtree_freeLiving.short_prefix.list,free_living
~/Taz.short_prefix.list,Taz" | \
sed 's/,/\t/g' | \
