...
cat ~/data/main_pipeline/annotations/eggnog-mapper_out/hog_annotations/hog_annotations.tsv > super_table.tsv
...
tail -n +2 | \
while read hog; do
cat ~/data/main_pipeline/hyphy_out/relax_out/results.tsv | \
awk -v hog=$hog '
	BEGIN {f=0};
...
orthofinder_dir=~/data/main_pipeline/orthofinder_out/for_downstream
echo "free_living
Taz" | \
while read group; do
