...
mkdir /home/liam/data/main_pipeline/hyphy_out
mkdir /home/liam/data/main_pipeline/macse_out

## Before running macse, double check that the nucleotide_HOG .ffn files are all unwrapped.
> tmp
ls /home/liam/data/main_pipeline/orthofinder_out/for_downstream/nucleotide_HOGs/*.ffn | \
while read file; do
echo $file >> tmp
a=$(grep -c "^>" $file)
b=$(wc -l < $file)
...
outdir=/home/liam/data/main_pipeline/macse_out
mkdir ${outdir}/trimmed_nucleotideHOGs
orthofinder_outdir=/home/liam/data/main_pipeline/orthofinder_out/for_downstream
...
mkdir /home/liam/data/main_pipeline/hyphy_out/msas
cd /home/liam/data/main_pipeline/hyphy_out/msas

ls /home/liam/data/main_pipeline/macse_out/*.fna | \
while read msa; do
og=${msa%.macse.fna}
og=${og##*/}
cat $msa | \
...
conda deactivate
indir=/home/liam/data/main_pipeline/hyphy_out/trees
cd $indir
...
bash -i /home/liam/scripts/helper/label_trees_not_ingroup_root.sh -i $in_tree -o $out_tree -l $logfile\
 -I /home/liam/Taz.short_prefix.list -O /home/liam/subtree_freeLiving.short_prefix.list
done
...
conda deactivate
msa_dir=/home/liam/data/main_pipeline/hyphy_out/msas
tree_dir=/home/liam/data/main_pipeline/hyphy_out/trees
outdir=/home/liam/data/main_pipeline/hyphy_out/relax_out
mkdir $outdir
cd $outdir
...
bash -i /home/liam/scripts/helper/hyphy_relax_exclamation_marks.sh\
 -m /home/liam/data/main_pipeline -I ~/Taz.short_prefix.list -O ~/subtree_freeLiving.short_prefix.list
...
## Run using different q values in /home/liam/data/main_pipeline/hyphy_out/tables_with_different_FDR_cutoffs
...
bash -i /home/liam/scripts/helper/hyphy_relax_categorical_analysis.sh
...
annotation_dir=/home/liam/data/main_pipeline/annotations/eggnog-mapper_out/hog_annotations/
> annotation_cols.tmp
cat results.tsv | \
awk -F "\t" '{print $1}' | \
while read hog; do
