...
outdir=~/data/main_pipeline/orthofinder_out
mkdir $outdir
cd $outdir
indir=input_fastas
...
initial_orthofinder_outdir=~/data/main_pipeline/orthofinder_out/initial_run/Results_Aug17
...
cd ~/data/main_pipeline/orthofinder_out
...
orthofinder_outdir=~/data/main_pipeline/orthofinder_out/final_run
...
cd ~/data/main_pipeline/orthofinder_out/final_run/N9_HOGs_locus_lists
...
cp ~/data/main_pipeline/pseudofinder_out/${long_prefix}/${long_prefix}_intact.ffn ./$file
## Unwrap
cat $file | \
awk ' {if ($0 ~ "^>") {printf "\n"$0"\n"} else { printf $0} }; END {printf "\n"} ' | \
tail -n +2 \
...
mkdir ~/data/main_pipeline/orthofinder_out/for_downstream
...
free_living,~/N9_freeLiving.shortened.tsv\
" | \
sed 's/,/\t/g' | \
