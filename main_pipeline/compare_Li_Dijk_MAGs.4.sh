...
cd ~/data/fastas
cp -r ~/data/assembly_workflow/metabat2_out/final_MAGs/second_assemblies/ ./Li_reassemblies
cd Li_reassemblies
...
mkdir ~/data/main_pipeline/Li_vs_Dijkhuizen
outdir=~/data/main_pipeline/Li_vs_Dijkhuizen/prokka_out
mkdir $outdir
indir=~/data/fastas/Li_reassemblies
...
## Pseudofinder
indir=~/data/main_pipeline/Li_vs_Dijkhuizen/prokka_out
outdir=~/data/main_pipeline/Li_vs_Dijkhuizen/pseudofinder_out
mkdir $outdir
conda deactivate
...
outdir=~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out
mkdir $outdir
indir=${outdir}/input_fastas
outdir=${outdir}/initial_run
## Get the inputs to the main pipeline orthofinder
cp -r ~/data/main_pipeline/orthofinder_out/input_fastas $indir
## Get the Li pseudofinder outputs
cat ~/Li.prefix.list | \
while read prefix; do
cp ~/data/main_pipeline/Li_vs_Dijkhuizen/pseudofinder_out/${prefix}/${prefix}_intact.faa ${indir}/${prefix}.fasta
done
## Run orthofinder!
...
initial_orthofinder_outdir=~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out/initial_run/Results_Aug21
...
orthofinder_outdir=~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out/final_run
orthofinder_downstream_dir=~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out/for_downstream
...
this_pseudofinder_outdir=~/data/main_pipeline/Li_vs_Dijkhuizen/pseudofinder_out
...
## Inputs:
outdir=~/data/main_pipeline/Li_vs_Dijkhuizen/assign_pseudogenes
N=N8
prefix_tsv=~/data/main_pipeline/Li_vs_Dijkhuizen/N8.shortened.tsv
pseudofinder_outdir=~/data/main_pipeline/Li_vs_Dijkhuizen/pseudofinder_out
orthofinder_downstream_dir=~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out/for_downstream
...
mkdir ~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out/for_downstream/aminoAcid_HOGs
mkdir ~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out/for_downstream/nucleotide_HOGs
## Check that there is a prefix on every header of the amino acid fastas.
cd ~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out/input_fastas
> tmp
...
Li_Dijk_analysis_dir=~/data/main_pipeline/Li_vs_Dijkhuizen/analyses
...
pseudofinder_dijk_dir=~/data/main_pipeline/pseudofinder_out/$dijk
pseudofinder_li_dir=~/data/main_pipeline/Li_vs_Dijkhuizen/pseudofinder_out/$li
...
echo "${dijk},${li}" >> $log
echo "" >> $log
dijk_genome=~/data/fastas/Dijkhuizen_genomes/${dijk}.fna
li_genome=~/data/fastas/Li_reassemblies/${li}.fna
...
cd ~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out/for_downstream/aminoAcid_HOGs
mkdir $dijk
cd $dijk
indir=~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out/input_fastas
cat ${Li_Dijk_analysis_dir}/${dijk}/dijk_li_single_copy.list | \
while read hog; do
> ${hog}.faa
grep "^${hog}" ~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out/for_downstream/N8_noOutgroups.tsv | \
...
cd ~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out/for_downstream/nucleotide_HOGs
mkdir $dijk
cd $dijk
dijk_indir=~/data/main_pipeline/pseudofinder_out/$dijk
li_indir=~/data/main_pipeline/Li_vs_Dijkhuizen/pseudofinder_out/$li
cat ${Li_Dijk_analysis_dir}/${dijk}/dijk_li_single_copy.list | \
while read hog; do
> ${hog}.ffn
grep "^${hog}" ~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out/for_downstream/N8_noOutgroups.tsv | \
...
> $outfile
indir=~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out/for_downstream/nucleotide_HOGs/$dijk
...
> $outfile
indir=~/data/main_pipeline/Li_vs_Dijkhuizen/orthofinder_out/for_downstream/aminoAcid_HOGs/$dijk
cat dijk_li_single_copy.list | \
while read hog; do
...
cd ~/data/main_pipeline/Li_vs_Dijkhuizen/analyses
