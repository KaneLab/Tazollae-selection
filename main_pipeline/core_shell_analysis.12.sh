...
main_dir=~/data/main_pipeline/core_vs_shell_analyses
mkdir $main_dir
cd $main_dir

## Loop through indirs
## binary tables by HOG, by KO, or by gene name
echo "~/data/main_pipeline/gene_loss,HOG
~/data/main_pipeline/inventory_by_annotation/KO,KO
~/data/main_pipeline/inventory_by_annotation/gene_name,gene_name" | \
sed 's/,/\t/g' | \
...
#paste ~/data/main_pipeline/gene_loss/Taz.intact_vs_not.tsv core_binary.tsv > venn_binary_Taz_intact_free_core.tsv
...
cat ~/data/main_pipeline/gene_loss/${group}.intact_vs_not.tsv | \
awk -F "\t" '
...
cd ~/data/main_pipeline/core_vs_shell_analyses/HOG
binary_infile=~/data/main_pipeline/gene_loss/all.intact_vs_not.tsv
...
cd ~/data/main_pipeline/core_vs_shell_analyses/HOG/intact
...
indir=~/data/main_pipeline/orthofinder_out/input_fastas
...
indir=~/data/main_pipeline/orthofinder_out/for_downstream/aminoAcid_HOGs
...
super_table=~/data/main_pipeline/super_table.tsv

## First, using HOG:
## How many such HOGs?
wc -l ~/data/main_pipeline/core_vs_shell_analyses/HOG/intact/free_core_Taz_not_core.list #66

## How much overlap with the ontology names that are core to all free-living but not T. az.?
grep -Ff ~/data/main_pipeline/core_vs_shell_analyses/HOG/intact/free_core_Taz_not_core.list $super_table | awk '{print $2}' | awk '$0!="-"' > names_of_HOGs.tmp
wc -l names_of_HOGs.tmp ##17 so 66-17=49 no name
grep -vFf ~/data/main_pipeline/core_vs_shell_analyses/gene_name/intact/free_core_Taz_not_core.list names_of_HOGs.tmp
## mutT
...
grep -Ff ~/data/main_pipeline/core_vs_shell_analyses/HOG/intact/free_core_Taz_not_core.list $super_table | awk '{print $3}' | awk '$0!="-"' > names_of_HOGs.tmp
wc -l names_of_HOGs.tmp #28 so 66-28=38 no KO
cat names_of_HOGs.tmp | tr "," "\n" | \
grep -vFf ~/data/main_pipeline/core_vs_shell_analyses/KO/intact/free_core_Taz_not_core.list
...
cat ~/data/main_pipeline/core_vs_shell_analyses/gene_name/intact/free_core_Taz_not_core.list | \
while read name; do
...
cat ~/data/main_pipeline/core_vs_shell_analyses/KO/intact/free_core_Taz_not_core.list kos_of_names.tmp | \
sort | uniq -c | \
awk '$1==2 {print $2}' \
...
cat ~/data/main_pipeline/core_vs_shell_analyses/KO/intact/free_core_Taz_not_core.list | \
while read ko; do
...
cat ~/data/main_pipeline/core_vs_shell_analyses/gene_name/intact/free_core_Taz_not_core.list names_of_kos.tmp | \
sort | uniq -c | \
awk '$1==2 {print $2}' \
...
grep -Ff ~/data/main_pipeline/core_vs_shell_analyses/gene_name/intact/free_core_Taz_not_core.list $super_table | less -S
